...
use reqenv::{SizePostfix, build_command, Options};
...
fn main() {
	...
	let cpu_speed = value_t!(matches, "cpu-speed", u8)
		.ok()
		.map(|s| s.min(100));
...
fn install_polkit_rule() -> ! {
	...
	std::fs::write(PATH, POLKIT_RULE)
		.expect("Unable to write file. Are you root/sudo?");
	std::process::exit(0)
}
