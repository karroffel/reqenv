...
impl Options {
    fn invariant(&self) -> bool {
        assert!(self.cpus.unwrap_or(1) > 0, "CPU count must be non-zero",);
        assert!(
...
#[post(opts.cpus.or(opts.cpu_speed).is_some()
	==> ret.arguments.iter().any(|e| e.starts_with("CPUQuota"))
)]
#[post(opts.memory.is_some()
	==> ret.arguments.iter().any(|e| e.starts_with("MemoryMax"))
)]
pub fn build_command(num_cpus: u8, opts: Options, command: &[String]) -> Command {
